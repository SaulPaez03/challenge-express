...
const { model } = require("../src/Controllers/apiControllers");
...
describe("model ", function () {
	...
	describe("must have a metod getAppointments, to view the appointments of a client", function () {
		...
		it("if a status was passed, should only return the appointments with that status", function () {
			let appointmentsPending = model.getAppointments(
				"javier",
				"pending"
			);
			let appointmentsAttended = model.getAppointments(
				"javier",
				"attended"
			);
			let appointmentsExpired = model.getAppointments(
				"alejandro",
				"expired"
			);
...
describe("server", () => {
			...
			return agent
				...
				.expect((res) => {
					expect(res.text).to.be.equal(
						"the body must have a client property"
					);
				});
		});
	...
	describe("GET /api/Appointments/:name?date=xxx&option=xxx", () => {
		...
		it("responds with a status 400 (bad request) and a string message, if the client does not have a appointment for this date", () => {
			return agent
				.get(
					"/api/Appointments/javier?date=23/10/2020%2014:00&option=attend"
				)
				.expect(400)
				.expect((res) => {
					expect(res.text).to.be.equal(
		...
		it("attend a appointment if the option passed by query is `attend`", () => {
			return agent
				.get(
					"/api/Appointments/javier?date=22/10/2020%2014:00&option=attend"
				)
				.expect(200)
				.expect((res) => {
		...
		it("expire a appointment if the option passed by query is `expire`", () => {
			return agent
				.get(
					"/api/Appointments/javier?date=22/10/2020%2016:00&option=expire"
				)
				.expect(200)
				.expect((res) => {
		...
		it("cancel a appointment if the option passed by query is `cancel`", () => {
			return agent
				.get(
					"/api/Appointments/javier?date=22/10/2020%2014:00&option=cancel"
				)
				.expect(200)
				.expect((res) => {
					expect(model.clients.javier[0]).to.be.deep.equal({
